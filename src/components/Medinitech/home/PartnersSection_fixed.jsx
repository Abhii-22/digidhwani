              ...
              <div className={`w-1/4 overflow-hidden relative h-[400px] ${theme === 'dark' ? 'bg-gradient-to-b from-gray-900 via-gray-800 to-gray-900' : 'bg-gradient-to-b from-white via-gray-50 to-white'}`}>
                ...
                >
                  {partnersColumn1.map((partner) => (
                    <div
                      key={partner.id}
                      className={`flex items-center justify-center p-4 ${theme === 'dark' ? 'bg-gray-800 border-amber-700' : 'bg-white border-amber-200'} rounded-lg shadow-md hover:shadow-lg border transition-all duration-300 w-full h-32 mx-auto my-4`}
                    >
                      <img 
                        src={partner.logo} 
                        alt={`${partner.name} logo`} 
                        className={`max-h-16 max-w-[80%] object-contain ${theme === 'dark' ? 'filter brightness-90 contrast-125' : ''}`}
                      />
                    </div>
                  ))}
                </div>
